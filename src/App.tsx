...
const code39SvgDataUrl = (value: string) => {
  ...
  return `data:image/svg+xml;utf8,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="64" viewBox="0 0 ${width} 64">${rects}</svg>`)}`;
};
...
export default function App() {
  ...
  const staffCardHtml = (card: any) => `<!doctype html><html><head><meta charset="utf-8"><title>${card.id} Staff ID</title><style>
    ...
    .main{position:absolute;left:5mm;right:5mm;bottom:9mm;display:grid;grid-template-columns:25mm 1fr 16mm;gap:3mm;align-items:end}.photoWrap{width:25mm}.photo{width:25mm;height:22mm;border:1px solid rgba(255,255,255,.35);border-radius:3mm;overflow:hidden;background:rgba(255,255,255,.12);display:grid;place-items:center;font-size:22px;font-weight:900}.photo img{width:100%;height:100%;object-fit:cover}.info{min-width:0}
    ...
    .qr{width:16mm;height:16mm;background:white;border-radius:1.5mm;padding:1.2mm;display:grid;place-items:center;align-self:center}.qr svg{width:100%;height:100%}.photoBar{width:25mm;height:7mm;margin-top:1mm;overflow:hidden}.photoBar img{width:100%;height:100%;object-fit:fill;display:block}.foot{position:absolute;left:5mm;right:5mm;bottom:3mm;border-top:1px solid rgba(255,255,255,.2);padding-top:1mm;display:flex;justify-content:space-between;font-size:6px;font-weight:700;color:rgba(255,255,255,.75)}
